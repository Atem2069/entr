...
void ARM7TDMI::Thumb_PCRelativeLoad()
{
	...
	uint32_t val = m_bus->NDS7_read32(PC + offset);
	R[destRegIdx] = val;
}
...
void ARM7TDMI::Thumb_LoadStoreRegisterOffset()
{
	...
	if (loadStore)	//load
	{
		if (byteWord)
		{
			uint32_t val = m_bus->NDS7_read8(base);
			R[srcDestRegIdx] = val;
		}
		else
		{
			uint32_t val = m_bus->NDS7_read32(base);
			if (base & 3)
...
void ARM7TDMI::Thumb_LoadStoreSignExtended()
{
	...
	if (op == 0)
	{
		...
		m_bus->NDS7_write16(addr, val);
	}
	else if (op == 2)	//load halfword
	{
		uint32_t val = m_bus->NDS7_read16(addr);
		if (addr & 0b1)
			val = std::rotr(val, 8);
		R[srcDestRegIdx] = val;
	}
	...
	{
		uint32_t val = m_bus->NDS7_read8(addr);
		if (((val >> 7) & 0b1))
			val |= 0xFFFFFF00;
		R[srcDestRegIdx] = val;
	}
	else if (op == 3)   //load sign extended halfword
	{
		...
		if (!(addr & 0b1))
		{
			val = m_bus->NDS7_read16(addr);
			if (((val >> 15) & 0b1))
				val |= 0xFFFF0000;
		}
		else
		{
			val = m_bus->NDS7_read8(addr);
			if (((val >> 7) & 0b1))
				val |= 0xFFFFFF00;
		}
		R[srcDestRegIdx] = val;
...
void ARM7TDMI::Thumb_LoadStoreImmediateOffset()
{
	...
	if (loadStore)	//Load value from memory
	{
		uint32_t val = 0;
		if (byteWord)
			val = m_bus->NDS7_read8(baseAddr);
		else
		{
			val = m_bus->NDS7_read32(baseAddr);
			if (baseAddr & 3)
	...
	else			//Store value to memory
	{
		uint32_t val = R[srcDestRegIdx];
		if (byteWord)
			m_bus->NDS7_write8(baseAddr, val & 0xFF);
		else
			m_bus->NDS7_write32(baseAddr, val);
	}
}
...
void ARM7TDMI::Thumb_LoadStoreHalfword()
{
	...
	if (loadStore)
	{
		uint32_t val = m_bus->NDS7_read16(base);
		if (base & 0b1)
			val = std::rotr(val, 8);
		R[srcDestRegIdx] = val;
	}
...
void ARM7TDMI::Thumb_SPRelativeLoadStore()
{
	...
	if (loadStore)
	{
		uint32_t val = m_bus->NDS7_read32(addr);
		if (addr & 3)
...
void ARM7TDMI::Thumb_PushPopRegisters()
{
	...
	if (loadStore) //Load - i.e. pop from stack
	{
		for (int i = 0; i < 8; i++)
		{
			...
			{
				transferCount++;
				uint32_t popVal = m_bus->NDS7_read32(SP);
				R[i] = popVal;
				SP += 4;
				firstTransfer = false;
			}
		...
		if (PCLR)
		{
			uint32_t newPC = m_bus->NDS7_read32(SP);
			setReg(15, newPC & ~0b1);
			SP += 4;
		}
	}
	else          //Store - i.e. push to stack
	{
		...
		{
			SP -= 4;
			m_bus->NDS7_write32(SP, getReg(14));
			firstTransfer = false;
		}
...
void ARM7TDMI::Thumb_MultipleLoadStore()
{
	...
	uint16_t regList = m_currentOpcode & 0xFF;

	bool writeback = true;								//writeback implied, except for some odd LDM behaviour
	bool baseIsFirst = false;
	int transferCount = 0;

	transferCount = __popcnt16(regList);
	baseIsFirst = ((regList >> baseRegIdx) & 0b1) && !((regList << (8 - baseRegIdx))&0xFF);
	...
	else
	{
		if (loadStore)
		{
			...
			setReg(15, m_bus->NDS7_read32(base));
		}
		else
		{
			m_bus->NDS7_write32(base, R[15] + 2);	//+2 for pipeline effect
		}
